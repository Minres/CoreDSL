...
class CoreDslAnalyzer {
    ...
    def static void analyzeAttribute(AnalysisContext ctx, Attribute attribute,
        ...
        if (info === null) {
            ...
            if (!info.allowedUsage.contains(expectedUsage)) {
                ctx.acceptError("Attribute '" + attribute.attributeName + "' cannot be placed here", attribute, null,
                    -1, IssueCodes.InvalidAttributePlacement);
            }
    ...
    def private static isValidRangeAlias(AnalysisContext ctx, Declarator aliasDeclarator, CoreDslType aliasType,
        ...
        val aliasSpaceElementType = if(aliasType instanceof AddressSpaceType) aliasType.
                elementType else if(aliasType instanceof ArrayType) aliasType.elementType else ErrorType.invalid
        val aliasSpaceCount = if(aliasType instanceof AddressSpaceType) aliasType.
                count else if(aliasType instanceof ArrayType) BigInteger.valueOf(aliasType.count) else BigInteger.ZERO
            ...
            IndexAccessExpression: {
                ...
                if (indexValue.isValid) {
                    if (targetType instanceof AddressSpaceType) {
                        ...
                        if (!targetType.isUnknownSize) {
                            checkIndexAccessBounds(ctx, indexValue.value, BigInteger.valueOf(targetType.count),
                ...
                if (expression.endIndex !== null) {
                    ...
                    if (endIndexValue.isValid) {
                        if (targetType instanceof AddressSpaceType) {
                            ...
                            checkIndexAccessBounds(ctx, endIndexValue.value, BigInteger.valueOf(targetType.count),
    ...
    def static dispatch CoreDslType analyzeExpression(AnalysisContext ctx, AssignmentExpression expression) {
        ...
        if (expression.operator != '=' && !ctx.isPartialAnalysis &&
            (!targetType.isIntegerType || !valueType.isIntegerType)) {
            ctx.acceptError(
        ...
        switch (expression.operator) {
            ...
            case '!=': {
                if (leftType.isAddressSpaceType || rightType.isAddressSpaceType) {
                    ctx.acceptError("Equality comparisons on address spaces are not allowed", expression,
                        CoreDslPackage.Literals.INFIX_EXPRESSION__OPERATOR, -1, IssueCodes.InvalidOperationType);
    ...
    def static dispatch CoreDslType analyzeExpression(AnalysisContext ctx, PrefixExpression expression) {
        var operandType = analyzeExpression(ctx, expression.operand);
        if (operandType.isIndeterminate && ctx.isPartialAnalysis)
            return ctx.setExpressionType(expression, operandType);
        switch (expression.operator) {
            case '++',
            case '--': {
